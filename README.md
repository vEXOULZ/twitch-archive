...
| Route / field | Notes |
|---|---|
| `GET /v1/games-played` | `[{name, gameId, image, imageTemplate, vods, chapters, seconds, watchableSeconds, lastPlayed}]`, one entry per game across all VODs' chapters. `vods` counts VODs (not chapters), `chapters` counts chapters, `seconds` is the game's total streamed time (the sum of its chapters' `end`, rounded to whole seconds; a missing or non-numeric `end` counts as 0), `watchableSeconds` the same without restricted (DMCA-cut) chapters, `lastPlayed` is the `createdAt` of the newest VOD with the game; `name`, `gameId` and `image` come from its most recent chapter. Grouped by `gameId`, else by name; chapters without a category are one entry named `No category` with `gameId: null`. Sorted by `vods` desc, `lastPlayed` desc, `name`. Cached like `/vods`. |
