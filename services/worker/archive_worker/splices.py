...
from .timeline import PlanError, Side
...
SPLIT_SUFFIX = "-{n}"  # the second half of VOD 123 is 123-2 (Twitch ids are digits only)
...
def _seconds(value: Decimal | float | None) -> int | float | None:
    if value is None:
        return None
...
def splice_json(sp: VodSplice) -> dict[str, Any]:
    return {
        ...
        "vodId": sp.vod_id,
        "otherId": sp.other_id,
        "offset": _seconds(sp.offset_s),
        "gap": _seconds(sp.gap_s),
        "detail": sp.detail,
...
async def _is_latest(s: AsyncSession, splice: VodSplice) -> VodSplice | None:
    """None if ``splice`` can be undone, else the later splice that has to be undone first."""
    later = [sp for sp in await _active(s, splice.vod_id, splice.other_id) if sp.id > splice.id]
    return later[-1] if later else None


async def _undoable(s: AsyncSession, splice: VodSplice) -> None:
    later = await _is_latest(s, splice)
...
async def _set_emotes(s: AsyncSession, vod_id: str, values: dict[str, Any] | None) -> None:
    """Replace ``vod_id``'s emotes row with ``values`` (EMOTE_COLUMNS; None: no row)."""
    row = await _emotes(s, vod_id)
    if values is None:
        if row is not None:
            await s.delete(row)
        return
...
async def merge(target_id: str, source_id: str, gap: Any = None) -> dict[str, Any]:
    ...
    async with _session() as s, s.begin():
        ...
        emotes_a, emotes_b = await _emotes(s, a.id), await _emotes(s, b.id)
        target_emotes = _emote_values(emotes_a)
        await _set_emotes(s, a.id, timeline.union_emotes(target_emotes, _emote_values(emotes_b)))

        games = list((await s.execute(select(Game.id).where(Game.vod_id == b.id).order_by(Game.id))).scalars())
        await s.execute(update(Game).where(Game.vod_id == b.id).values(
            vod_id=a.id, start_time=Game.start_time + offset, end_time=Game.end_time + offset))

        splice = VodSplice(kind="merge", vod_id=a.id, other_id=b.id, offset_s=offset, gap_s=offset - side_a.duration,
                           detail={}, snapshot={})
        s.add(splice)
        await s.flush()
        ...
        a.duration, a.chapters, a.youtube, a.drive = format_hhmmss(plan.duration), plan.chapters, plan.youtube, plan.drive
        a.chapters_locked = True  # the automatic chapters step would put Twitch's (A-only) chapters back
...
async def _unmerge(s: AsyncSession, splice: VodSplice, force: bool) -> dict[str, Any]:
    ...
    await _set_emotes(s, a.id, snap["targetEmotes"])
    await _unpoint(s, snap["repointed"])
    _restore(a, snap["target"])
...
async def unmerge(target_id: str, source_id: str, force: bool = False) -> dict[str, Any]:
    async with _session() as s, s.begin():
        ...
        return await _unmerge(s, splice, force)
...
async def _free_id(s: AsyncSession, vod_id: str) -> str:
    ...
    n = 2
    while vod_id + SPLIT_SUFFIX.format(n=n) in taken:
        n += 1
    return vod_id + SPLIT_SUFFIX.format(n=n)
...
async def split(vod_id: str, at: Any, force: bool = False) -> dict[str, Any]:
    ...
    async with _session() as s, s.begin():
        ...
        for merge_ in reversed([sp for sp in await _active(s, a.id) if sp.kind == "merge" and sp.vod_id == a.id]):
            lo, hi = _join(merge_)
            if lo - timeline.SPLIT_SLACK <= at <= hi + timeline.SPLIT_SLACK and await _is_latest(s, merge_) is None:
                return {"undid": "merge", **await _unmerge(s, merge_, force)}
        ...
        await _set_emotes(s, new_id, _emote_values(await _emotes(s, a.id)))
        games = list((await s.execute(select(Game.id).where(
            Game.vod_id == a.id, Game.start_time >= cut).order_by(Game.id))).scalars())
        if games:
            await s.execute(update(Game).where(Game.id.in_(games)).values(
        ...
        a.duration, a.chapters, a.youtube, a.drive = format_hhmmss(first.duration), first.chapters, first.youtube, first.drive
        a.chapters_locked = True
...
async def unsplit(vod_id: str, other_id: str | None = None, force: bool = False) -> dict[str, Any]:
    ...
    async with _session() as s, s.begin():
        ...
        a, new = await _lock(s, splice.vod_id, splice.other_id)
        await _undoable(s, splice)
        await _no_active_jobs(s, a.id, new.id)
        _unedited(splice, {a.id: a, new.id: new}, force)
        ...
        await _set_emotes(s, new.id, None)
        await s.delete(new)
...
async def active_splices(vod_id: str) -> list[dict[str, Any]]:
    ...
    async with _session() as s:
        out = []
        for sp in await _active(s, vod_id):
            out.append({**splice_json(sp), "undoable": await _is_latest(s, sp) is None})
        return out
